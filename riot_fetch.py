...
def sync_player(cur, conn, headers, riot_name, riot_tag, anzahl_matches=20):
    ...
    for match_id in neue_match_ids:
        ...
        for p in info["participants"]:
            if p["puuid"] == puuid:
                # Team-Summen für Kill-Participation & Damage-Share (relativ zum Team, nicht absolut)
                ...
                # Turm-Takedowns & gestohlene Objectives sind Top-Level-Felder, Solo-Kills kommt
                # aus dem von Riot bereits vorgerechneten "challenges"-Objekt (kein Extra-API-Call nötig).
                ...
                # Items (7 Slots inkl. Trinket) & Runen - für die Postgame-Detailansicht
                items = [p.get(f"item{i}", 0) for i in range(7)]
                perks = p.get("perks", {})
                champ_level = p.get("champLevel", 0)

                # Damage-Rang unter allen 10 Spielern (steckt schon in info["participants"],
                # kein Extra-Call nötig) und Gold-Diff zum Lane-Gegner (gleiche teamPosition,
                # anderes Team - in ARAM ohne Rollen bleibt das None).
                alle_spieler = info["participants"]
                damage_sortiert = sorted(
                    alle_spieler, key=lambda pp: pp["totalDamageDealtToChampions"], reverse=True
                )
                damage_rank = next(
                    i for i, pp in enumerate(damage_sortiert, start=1) if pp["puuid"] == p["puuid"]
                )
                lane_gegner = next(
                    (pp for pp in alle_spieler
                     if pp["teamId"] != p["teamId"] and p["teamPosition"]
                     and pp["teamPosition"] == p["teamPosition"]),
                    None
                )
                ...
                """, (
                    match_id, puuid, p["championName"], p["teamPosition"], p["win"],
